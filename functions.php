...
add_filter('category_link', 'fix_category_parents_path', 1000, 2);
function fix_category_parents_path($catlink, $category_id) {
    ...
    // Loop through parents to get full path
    while ($category->parent != 0) {
        $parent_category = get_category($category->parent);
        $full_path = $parent_category->slug . '/' . $full_path;
        $category = $parent_category;
    }

    // Build the new category link
    $catlink = trailingslashit(get_option('home')) . user_trailingslashit($full_path . $category_nicename, 'category');
    return $catlink;
}

// Update category rewrite rules
add_filter('category_rewrite_rules', 'fix_category_rewrite_rules');
function fix_category_rewrite_rules($category_rewrite) {
    ...
    foreach ($categories as $category) {
        ...
        $category_rewrite['(' . $full_path . $category_nicename . ')/?$'] = 'index.php?category_name=' . $full_path . $category_nicename;
    }
