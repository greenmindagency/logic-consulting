...
<div class="articletitle container contentfield<?php $center = get_sub_field('center'); if( $center && in_array('center', $center) ) { ?> text-center<?php } else { ?><?php } ?>">
...
$articleimages = get_sub_field('articleimages'); 
?>


  <div class="row mt-3 mb-4 row-cols-1 row-cols-sm-<?php echo esc_attr($imagecolumn); ?> row-cols-lg-<?php echo esc_attr($imagecolumn); ?>" <?php if (is_array($articleimages) && count($articleimages) > 1): ?> data-masonry='{"percentPosition": true }'<?php endif; ?>>
  

<?php $images = get_sub_field('articleimages'); if( $images ): $alt = $image['alt']; ?>
...
<img class="m-0 img-fluid imgwidthfull lazyload<?php $style = get_sub_field('style'); if (get_sub_field('style')) {?> <?php echo $style; ?><?php } ?>" src="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=" data-src="<?php echo $image['sizes'][$imagesize]; ?>" alt="<?php if( !empty($image['alt']) ): ?><?php echo $image['alt']; ?><?php else: ?><?php the_title(); ?><?php endif; ?>" width="<?php echo $width?>" height="<?php echo $height?>" >
...
<?php while ( have_rows('columns_group') ) : the_row(); $column_position = get_sub_field('column_position');?>
<?php if (strpos($column_position, 'start') !== false) { //if cond. ?><div class="container"><div class="row"> 
<?php } ?>
<?php endwhile; ?>
<!-- //define if column start -->
...
<div class="verticaltabs container-fluid py-spacer <?php while ( have_rows('columns_group') ) : the_row(); $column_container = get_sub_field('column_container'); $column_width = get_sub_field('column_width'); ?><?php echo $column_container ?><?php echo $column_width ?> <?php endwhile; ?>">
...
<?php endif; ?>
  ...
  <div class="row">
	...
	<div class="col-md-8" data-aos="flip-up" data-aos-delay="<?php echo $delay; ?>">
  <?php if( have_rows('verticaltabs') ): $isActive = true; ?>
    <div class="tab-content" id="v-pills-tabContent">
      <?php while( have_rows('verticaltabs') ) : the_row(); ?>
       ...
       <?php $delay += 300; // Increase delay by 200ms for each item for animation ?>
...
<div class="col-md-4">
<h4 class="fw-bolder"><?php if ($icon) : ?><i class="me-3 <?php echo ($icon); ?>"></i><?php endif; ?><?php echo $title ?></h4>
...
<div class="col-md-3 mb-4">
    ...
    $image = get_sub_field('image');
    if (!empty($image)):
        $size = 'medium'; // The selected image size
...
<div class="container-fluid position-relative h-100"  data-aos="fade-left" data-aos-duration="1000" data-aos-easing="ease-in">
...
<div class="container py-spacer pagecontent9">
  ...
  <?php if ($pagecontent9_slider && in_array('slider', $pagecontent9_slider)) : ?>
    <div id="slider<?php echo $sliderCounter; ?>" class="carousel slide" data-bs-ride="carousel" data-bs-interval="7000">
        ...
        <rect width="100%" height="100%" fill="#f8f9fb"/>
                    ...
                    <div class="d-flex flex-column justify-content-start p-4 card-overlay">
                      ...
                      <div class="hover-opacity hover-link-container">
                        <a href="<?php echo esc_url($card['read_more_link']); ?>" class="text-warning text-decoration-none small fw-semibold">
    ...
    <div class="my-5 container">
        ...
        <div class="justify-content-center row">
            ...
            <?php foreach ($catss as $cat) { ?>
                ...
                $description = get_field('description', $cat);
                $tag_link = get_tag_link($cat);
                ?>
                ...
                <?php $delay += 100; // Increase delay for animation ?>
            <?php } wp_reset_query(); ?>
        </div>
        <!-- show selected categories -->
    </div>
...
<?php
// Global counter (same as your working slider)
global $sliderCounter;
if (!isset($sliderCounter)) {
  $sliderCounter = 1;
}
...
<div id="slider<?php echo $sliderCounter; ?>" data-bs-pause="false" data-bs-interval="7000" class="lazy-load carousel keyboard touch slide d-flex align-items-center slider-height-<?php echo get_sub_field('height'); ?> p-0" data-bs-ride="carousel">
  ...
  <div class="carousel-indicators">
    <ul class="list-unstyled d-flex justify-content-center m-0 p-0">
      <?php 
      $catss = get_sub_field('cats');
      $total_slides = ceil(count($catss) / 3); // Divide by 3 for 3 items per slide
  ...
  <div class="carousel-inner my-3">
    <?php
    $counter = 0;
    foreach ($catss as $index => $cat):
      $icon = get_field('icon', $cat);
      $description = get_field('description', $cat);
      $tag_link = get_tag_link($cat);

      if ($counter % 3 == 0): // Open new slide every 3 items
    ?>
      <div class="carousel-item <?php if($counter == 0) echo 'active'; ?> mb-5">
    ...
    <?php 
      $counter++;
      if ($counter % 3 == 0 || $counter == count($catss)): // Close the slide after 3 items or last item
    ?>
        </div>
      </div>
    <?php endif; ?>
    ...
    <ul class="list-unstyled d-flex justify-content-center m-0 p-0">
      ...
      foreach ($termss as $postsrelatedcat) {
        $args = array(
          ...
          'posts_per_page' => get_sub_field('postscount')
        );
...
// Exclude current post only on single post pages
if ( is_single() ) {
...
<?php foreach ($termsss as $catconnection) { // let the loop understand the values you would like to echo  ?>
 ...
 <div class="catconnection container-fluid bg-secondary">
   ...
   <div class="col-md-6 text-white px-spacer py-spacer" data-aos="fade-right">
...
<?php elseif( get_row_layout() == 'columnswithImages' ): ?>

<div class="columnswithImages py-spacer">
    <div class="text-center">
  <h3 class="fs-1 fw-bold">
    <?php $title = get_sub_field('title'); if ($title) echo esc_html($title); ?>
  </h3>
...
<?php if (have_rows('columnswithImages')) : ?>

<div class="container">
  <div class="row">
    <?php while (have_rows('columnswithImages')) : the_row(); 
    ...
    ?>
      ...
      <div class="col-md-4" data-aos="fade-down" data-aos-delay="<?php echo $delay; ?>">
        <div class="card shadow h-100 p-3">
		
		<?php 
    
    if (!empty($card_image)):
        $size = 'large'; // The selected image size
        $image_url = $card_image['sizes'][$size];
        $width = $card_image['sizes'][$size . '-width'];
        $height = $card_image['sizes'][$size . '-height'];
    ...
    <?php endif; ?>
          ...
          <div class="card-body d-flex flex-column">
            <?php if ($card_title): ?>
              <h5 class="mt-0 card-title fw-bold"><?php echo esc_html($card_title); ?></h5>
            <?php endif; ?>
            ...
            <?php if ($card_link): ?>
              <div class="read-more-wrapper mt-auto d-flex justify-content-end align-items-center mt-3">
                <i class="fa-solid fa-caret-right me-2"></i> <a href="<?php echo esc_url($card_link); ?>" class="text-decoration-none fw-medium text-success">Read More</a>
              </div>
            <?php endif; ?>
          </div>
        </div>
